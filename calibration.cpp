...
bool Calibration::performCalibration(std::string str){
	while(true){
		...
		// if current camera is calibrated, then rotate each point and save to local file
		else{
			if(!save2Local){
				...
				for(int i=0; i<packet->height_d; i++){
					for(int j=0; j<packet->width_d; j++){
						...
						if(tempPoint.Z > 0 && tempPoint.Z < 8){
				...
				std::string plyFileName = "pc_" + str + ".ply";
				savePlyFile(plyFileName.c_str(), vertices, false, colors);
				save2Local = true;
			}
...
// 	res[0] = point.X * R[0][0] + point.Y * R[0][1] + point.Z * R[0][2] + T[0];
// 	res[1] = point.X * R[1][0] + point.Y * R[1][1] + point.Z * R[1][2] + T[1];
// 	res[2] = point.X * R[2][0] + point.Y * R[2][1] + point.Z * R[2][2] + T[2];
